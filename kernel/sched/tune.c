 ...
 */
static struct threshold_params threshold_gains[] = {
    {0, 5}, /*   < 10% */
    {1, 5}, /*   < 20% */
    {2, 5}, /*   < 30% */
    {3, 5}, /*   < 40% */
...
static int __schedtune_accept_deltas(int nrg_delta, int cap_delta,
                                     int perf_boost_idx,
 ...
 *                                                        |
 *                                             SLOW PATH  |   FAST PATH
 *                              CGroup add/update/remove  |   Scheduler
 ...
 *                                                        |     |  idle        |
 * |   |    |    | |     |  boost_max   |    |   |    |    | |  +---->lock | |
 * |    |    | struct schedtune                  allocated_groups    |  |  |
 * group[    ] |    |   |    |    |
 *  +------------------------------+         +-------+    |  |
 * +--+---------+-+----+---+----+----+ | idx                          | | |    |
 * |     |  valid  | | boots / prefer_idle          |         |       |    |  |
 * |  boost  | | perf_{boost/constraints}_idx | <---------+(*)  |    |  |     |
 * tasks  | <------------+ | css                          |         +-------+ |
 * |     +---------+              |
 *  +-+----------------------------+         |       |    |  |     |         | |
 *    ^                                      |       |    |  |     |         | |
...
int schedtune_accept_deltas(int nrg_delta, int cap_delta,
  ...
  return __schedtune_accept_deltas(nrg_delta, cap_delta, perf_boost_idx,
                                   perf_constrain_idx);
}
...
static void schedtune_cpu_update(int cpu) {
...
static inline void schedtune_tasks_update(struct task_struct *p, int cpu,
  ...
  trace_sched_tune_tasks_update(p, cpu, tasks, idx, bg->group[idx].boost,
                                bg->boost_max);
...
static s64 boost_read(struct cgroup_subsys_state *css, struct cftype *cft) {
  struct schedtune *st = css_st(css);
...
#ifdef CONFIG_DYNAMIC_STUNE_BOOST
static s64 sched_boost_read(struct cgroup_subsys_state *css,
                            struct cftype *cft) {
  struct schedtune *st = css_st(css);
...
static int sched_boost_write(struct cgroup_subsys_state *css,
                             struct cftype *cft, s64 sched_boost) {
  struct schedtune *st = css_st(css);
...
static void boost_slots_init(struct schedtune *st) {
  int i;
...
static void boost_slots_release(struct schedtune *st) {
  struct boost_slot *slot, *next_slot;

  list_for_each_entry_safe(slot, next_slot, &(st->available_boost_slots.list),
                           list) {
    list_del(&slot->list);
    pr_info("STUNE: freed!\n");
    kfree(slot);
  }
  list_for_each_entry_safe(slot, next_slot, &(st->active_boost_slots.list),
                           list) {
    list_del(&slot->list);
    pr_info("STUNE: freed!\n");
    kfree(slot);
...
static int deactivate_boost_slot(struct schedtune *st, int slot) {
  ...
  list_for_each_entry_safe(curr_slot, next_slot, &(st->active_boost_slots.list),
                           list) {
    if (curr_slot->idx == slot) {
      ...
      curr_slot = kmalloc(sizeof(*curr_slot), GFP_KERNEL);
      curr_slot->idx = slot;
      list_add_tail(&(curr_slot->list), &(st->available_boost_slots.list));
...
static void schedtune_test_nrg(unsigned long delta_pwr) {
  ...
  for (idx = 0; idx < 6; ++idx) {
    ...
    test_norm_pwr = reciprocal_divide(test_delta_pwr << SCHED_CAPACITY_SHIFT,
                                      schedtune_target_nrg.rdiv);

    pr_info("schedtune: max_pwr/2^%d: %4lu => norm_pwr: %5lu\n", idx,
            test_delta_pwr, test_norm_pwr);
  }
}
#else
 ...
 */
static void schedtune_add_cluster_nrg(struct sched_domain *sd,
